...
public class ItemService {
	...
	public void setSessionFactory(SessionFactory sessionFactory) {
		...
		questionTypeDao = new HibernateDao<QuestionType, String>(sessionFactory,
				QuestionType.class);
		itemQueueDao = new HibernateDao<ItemQueue,String>(sessionFactory, ItemQueue.class);
		itemQuestionTypeDao = new HibernateDao<ItemQuestionType,String>(sessionFactory, ItemQuestionType.class);
	}
	...
	public Item uploadImageFirst(File image) throws IOException {
		...
		position = PicExifUtil.readGps(new BufferedInputStream(new FileInputStream(image)));
		if (position != null) {
		...
		staticServerService.uploadFile(item.getImage().getId(), StringUtils.lowerCase(FilenameUtils.getExtension(image.getName())), FileUtils.readFileToByteArray(image));
	...
	public Item uploadImageFirst(MultipartFile image) throws IOException {
		...
		double[] position = null;
		position = PicExifUtil.readGps(new BufferedInputStream(image.getInputStream()));
		if (position != null) {
			item.setItemLat(position[0]);
			item.setItemLng(position[1]);
		}
		...
		item.setDisabled(1);//Disable
		itemDao.save(item);
		staticServerService.uploadFile(item.getImage().getId(), StringUtils.lowerCase(FilenameUtils.getExtension(image.getOriginalFilename())), image.getBytes());
		if("image".equals(fileData.getFileType())){
			List<FileData> fileDataList = new ArrayList<FileData>();
			fileDataList.add(fileData);
		}
	...
	public Item uploadImageFirst(SensePic pic) throws IOException {
		...
		double[] position = null;
		position = PicExifUtil.readGps(new BufferedInputStream(file.getInputStream()));
		if (position != null) {
			item.setItemLat(position[0]);
			item.setItemLng(position[1]);
		}
		...
		staticServerService.uploadFile(item.getImage().getId(), StringUtils.lowerCase(FilenameUtils.getExtension(file.getFilename())), fileBytes);
		if("image".equals(fileData.getFileType())){
			List<FileData> fileDataList = new ArrayList<FileData>();
			fileDataList.add(fileData);
		}
...
//				image.setFileBin(bin);
	    ...
	    if(quesTypeIds!=null&&quesTypeIds.contains(Constants.QuizTypeImageMutiChoice)&&!imageFlg){
		...
		for(String key: form.getTitleMap().keySet()){
			...
			if(lang==null || StringUtils.isBlank(form.getTitleMap().get(key)))continue;
			...
			if(quesTypeIds!=null&&quesTypeIds.contains(Constants.QuizTypeImageMutiChoice)){
				ItemQuestionType qt = new ItemQuestionType();
				qt.setItem(item);
				qt.setLanguage(lang);
				qt.setQuestionType(this.questionTypeDao.findUniqueBy("id", Constants.QuizTypeImageMutiChoice));
				quesTypes.add(qt);
			}
			if(quesTypeIds!=null&&quesTypeIds.contains(Constants.QuizTypeTextMutiChoice)){
				ItemQuestionType qt = new ItemQuestionType();
				qt.setItem(item);
				qt.setLanguage(lang);
				qt.setQuestionType(this.questionTypeDao.findUniqueBy("id", Constants.QuizTypeTextMutiChoice));
				quesTypes.add(qt);
			}
		}

		if(quesTypeIds!=null&&quesTypeIds.contains(Constants.QuizTypeYesNoQuestion)){
			ItemQuestionType qt = new ItemQuestionType();
			qt.setItem(item);
			qt.setQuestionType(this.questionTypeDao.findUniqueBy("id", Constants.QuizTypeYesNoQuestion));
			quesTypes.add(qt);
		}
		...
		if (!StringUtils.isBlank(form.getQuestion())) {
			...
			question.setLanguage(languageDao.findUniqueBy("code", form
					.getQuesLan()));
		...
		if(form.getLocationBased()!=null&&form.getLocationBased().booleanValue()){
			locationFlg = GeoUtils.isHighPrecision(item.getItemLat(), item.getItemLng());
		}
		...
		itemQueueService.updateItemQueue(item, user, QuizConstants.QueueTypeNewObject);
		reviewHistoryService.updateItemState(item, user, Constants.ExperiencedState, null);

		if (form.getImage() != null && !form.getImage().isEmpty()) {
			staticServerService.uploadFile(item.getImage().getId(), StringUtils.lowerCase(FilenameUtils.getExtension(form.getImage().getOriginalFilename())), form.getImage().getBytes());
...
//				image.setFileBin(bin);
		...
		List<Long>quesTypeIds = form.getQuestionTypeIds();
		if(!FilenameUtil.IMAGE.equals(fileType)&&quesTypeIds!=null&&quesTypeIds.contains(Constants.QuizTypeImageMutiChoice)){
			quesTypeIds.remove(Constants.QuizTypeImageMutiChoice);
		}
		...
		if (form.getQuestion() == null || form.getQuestion().length() <= 0) {
			...
			question.setContent(form.getQuestion());
			Language language = this.languageDao.findUniqueBy("code", form
					.getQuesLan());
...
//					QuestionType qt = this.questionTypeDao.findUniqueBy("id", quesTypeId);
//					if(qt!=null){
	...
	public void createCommentByForm(String itemId, ItemCommentForm form) {
		...
		if (!StringUtils.isBlank(form.getComment())) {
			...
			try {
				...
				model.addAttribute("url", propertyService.getSystemUrl() + "/item/" + item.getId());
	...
	public void createAnswerByForm(String itemId, String content) {
		...
		if(!user.getId().equals(item.getAuthor().getId())){
			try {
				...
				mailService.sendSysMail(item.getAuthor().getPcEmail(),
						"Your question has been answered", "questionAnsweredMail",
						model);
			} catch (MessagingException ex) {
				ex.printStackTrace();
			} catch (IOException ex) {
...
//	}
	 ...
	 * @param userId Nullable, if it is null, search in all users' items
	 * @param itemSize
	 * @return
	 */
	@Transactional(readOnly = true)
	public Page<Item> searchNearestItems(String avoidItemId, double lat, double lng, double distance, String userId, int itemSize){
		...
		String sql = "select "
			...
			+"	from"
			+"		("
			+"		select i.*, get_distance("+lat+","+lng+", i.item_lat, i.item_lng) as distance"
			+"		from t_item i"
			+"		where i.disabled!=1 and i.item_lat is not null and i.item_lng is not null and i.relog_item is null"+userCond+avoidCond
			+"		order by distance asc"
			+"	) as t"
			+"	where"
			+"		t.distance>=0 and t.distance<="+distance
			+" limit 0, "+itemSize;
		Page<Item> itemPage = new Page<Item>(itemSize);

		Session session = this.sessionFactory.getCurrentSession();
		@SuppressWarnings("unchecked")
		List<Item> itemList = session.createSQLQuery(sql).addEntity(Item.class).list();
	...
	@SuppressWarnings("unchecked")
	public List<Item> searchNearestItemsWithoutNotified(double lat, double lng, double distance, int itemSize){
		Users user = userService.getById(SecurityUserHolder.getCurrentUser()
				.getId());
		String sql = "select "
			+" t.*"
			...
			+"	where ia.author_id = '"+user.getId()+"' and i.id = ia.item and ia.alarm_type = 0)"
			+"		order by distance asc"
			+"	) as t"
			+"	where"
			+"		t.distance>=0 and t.distance<="+distance
	 ...
	 * @param userId Nullable, if it is null, search in all users' items
	 * @param itemSize more than 0
	 * @return
	 */
	@Transactional(readOnly = true)
	public Page<Item> searchLatestItems(String avoidItemId, Long userId, int itemSize){
		if(itemSize==0) itemSize=1;
		DetachedCriteria criteria = DetachedCriteria.forClass(Item.class);
		criteria.add(Restrictions.ne("disabled", new Integer(1)));
		if(userId!=null){
		...
		criteria.add(Restrictions.isNull("relogItem"));
		if(avoidItemId!=null) criteria.add(Restrictions.ne("id", avoidItemId));
		Page<Item> itemPage = new Page<Item>(itemSize);
		itemPage.setOrder(Page.DESC);
	...
	private DetachedCriteria buildCriteriaByForm(ItemSearchCondForm form) {
		...
		if (!StringUtils.isBlank(form.getTitle())) {
			criteria.createAlias("titles", "title").add(
					Restrictions.like("title.content", form.getTitle(), MatchMode.ANYWHERE));
...
//			}
		...
		if (!StringUtils.isBlank(form.getAnsweruserId())) {
			...
			criteria.createAlias("question", "question").createAlias(
					"question.answerSet", "answer").add(
					Restrictions.eq("answer.author", answerAuthor));
		} else if (!StringUtils.isBlank(form.getAnsweruser())) {
			Users answerAuthor = this.userService.findUniqueBy("nickname", form
					.getAnsweruser());
			if (answerAuthor == null) {
				return null;
			}
		...
		if (!StringUtils.isBlank(form.getQuestionStatus())) {
			if ("inquestion".equals(form.getQuestionStatus())) {
				criteria.add(Restrictions.and(Restrictions
						.isNotNull("question"), Restrictions.or(Restrictions
						.isNull("questionResolved"), Restrictions.eq(
						"questionResolved", Boolean.FALSE))));
			} else if ("resolved".equals(form.getQuestionStatus())) {
				criteria.add(Restrictions.or(Restrictions.eq(
						"questionResolved", Boolean.TRUE), Restrictions
						.isNull("question")));
			}
		}
		...
		if (form.getToAnswerQuesLangs() != null
			...
			criteria.createAlias("question", "question").add(
					Restrictions.and(
							...
							Restrictions.in("question.language", form
									.getToAnswerQuesLangs())));
			...
			if (langSet.size() > 0) {
				criteria.createAlias("question", "question").add(
						Restrictions.and(Restrictions
								.isEmpty("question.answerSet"), Restrictions
								.in("question.language", langSet)));
			}
		}
		...
		if (form.getToStudyQuesLangs() != null
			...
			if (langSet.size() > 0) {
				criteria.createAlias("question", "question").add(
						Restrictions.and(Restrictions
								.isNotEmpty("question.answerSet"), Restrictions
								.in("question.language", langSet)));
			}
		}
	...
	public void delete(Item item) {
		Item i = itemDao.get(item.getId());
		if (i.getRelogItem() != null) {
			i.setImage(null);
			i.setRelogItem(null);
		}
		itemDao.delete(i);
	...
	public Page<Item> searchRelatedItemList(String itemId) {
		...
		for(String t: titlesCond){
			...
			criteria.add(Restrictions.like("title.content", t, MatchMode.ANYWHERE));
	 ...
	 * @param months
	 * @param inWeeks Nullable
	 * @param userId Nullable
	 * @param size Default 20
	 * @return
	 */
	public List<Item> findAllItemsBeforeMonths(int months, Integer inWeeks, String userId, Integer size){
		DetachedCriteria criteria = DetachedCriteria.forClass(Item.class);
		...
		if(inWeeks!=null){
			cal.set(Calendar.WEEK_OF_YEAR, cal.get(Calendar.WEEK_OF_YEAR)-inWeeks);
			Date start = cal.getTime();
			criteria.add(Restrictions.and(Restrictions.ge("createTime", end), Restrictions.le("createTime", start)));
		}else{
	...
	@SuppressWarnings("unchecked")
	public List<Item> findItemByLocation(Double lat, Double lng, Boolean isMyItem) throws Exception {
		...
		if(Boolean.TRUE.equals(isMyItem)){
			...
			"and i.item_lng<='"+y1+"' and i.item_lng>='"+y2+"' and i.author_id = '"+user.getId()+"' "+
			"and i.id not in ( "+
					"select inm.items "+
		...
		if(Constants.usingCategory && user.getMyCategoryList()!=null&&user.getMyCategoryList().size()>0){
			String categoryids = "";
	...
	public Integer getAnswerCount(String id) {
		...
		Integer result = ((BigInteger)session.createSQLQuery(sql).setParameter("id", id).uniqueResult()).intValue();
