...
public class LLQuizService {
    ...
    public QuizWrapper findQuizWrapper(QuizCondition quizCon) {
    	...
    	if (quiz != null ) {
    		QuizGenerator quizGenerator= quizGeneratorFactory.getQuizGenerator(quiz.getQuestionType());
    ...
    public List<QuizForm> findAllSyncQuizzes(QuizCondition quizCon) {
		...
		List<ItemQueue> itemqueues = this.sessionFactory.getCurrentSession().createSQLQuery(sql).addEntity(ItemQueue.class).list();
    	...
    	if(quizNumber<20){
    		if(quizCon.getIsRecommended()!=null&&quizCon.getIsRecommended().booleanValue()){
    			List<ItemQueue> queues = this.itemQueueService.searchUserRecommendedItems(userService.getById(SecurityUserHolder.getCurrentUser()
    					.getId()));
...
//		Set<QuestionType> questionTypes = item.getQuestionTypes();
		...
		dc_qt.add(Restrictions.or(Restrictions.in("language", user.getStudyLangs()), Restrictions.isNull("language")));
		dc_qt.addOrder(Order.asc("questionType"));
		List<ItemQuestionType> questionTypes = this.itemQuestionTypeDao.find(dc_qt);
		MyQuiz myquiz = null;
...
//		}
		...
		for(ItemQuestionType qt:questionTypes){
			QuizGenerator quizGenerator= quizGeneratorFactory.getQuizGenerator(qt.getQuestionType());
			if(quizGenerator==null)
				continue;
	...
	private boolean checkQuizQuality(MyQuiz quiz){
		if(quiz.getQuestionType().getId().equals(Constants.QuizTypeImageMutiChoice)){
	...
	public MyQuiz checkQuiz(QuizCondition quizCon){
       ...
       if(quizCon.getLng()!=null&&quizCon.getLat()!=null){
		...
		QuizGenerator quizGenerator= quizGeneratorFactory.getQuizGenerator(myquiz);
		if(quizGenerator==null)
			return null;
